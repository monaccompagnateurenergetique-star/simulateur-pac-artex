...
import {
  ...
  Loader2, Thermometer, Search,
  Bell, Check, Calendar, CheckCircle, Layers, User, Zap,
  Clock, MoreHorizontal, Copy, PhoneCall, MailPlus, Globe
} from 'lucide-react'
import { useProjects, PROJECT_STATUSES, TYPES_TRAVAUX, getTypeTravauxInfo } from '../hooks/useProjects'
        ...
        return (
          ...
          >
            {isPast ? (
              ...
              <div className={`w-3 h-3 rounded-full shrink-0 ring-[3px] ring-offset-1 ${s.dot} ring-indigo-200`} />
            ) : (
...
export default function ClientDetailPage() {
  ...
  const [mainTab, setMainTab] = useState('scenarios')
  ...
  async function handleSearchDpe() {
    ...
    setShowDpe(true)
    try {
      const { results, audits } = await searchDPE(project.address || '', project.postalCode || '', project.city || '')
      setDpeResults(results)
      setAuditResults(audits || [])
      // Persister audits si DPE deja rattache (pas besoin de re-selectionner)
      if (project.dpe && audits && audits.length > 0) {
        updateProject(id, { audits })
      }
    } catch {
      /* ignore */
    } finally {
  ...
  function selectDpe(d) {
    updateProject(id, {
      dpe: {
        ...
        surface: d.surface, surfaceImmeuble: d.surfaceImmeuble,
        typeBatiment: d.typeBatiment, nombreAppartements: d.nombreAppartements,
        nombreNiveaux: d.nombreNiveaux,
        consoM2: d.consoM2, emissionGes: d.emissionGes,
        coutTotal: d.coutTotal, coutChauffage: d.coutChauffage,
        installationChauffage: d.installationChauffage,
        energieChauffage: d.energieChauffage, generateurChauffage: d.generateurChauffage,
        installationEcs: d.installationEcs, energieEcs: d.energieEcs,
        typeVentilation: d.typeVentilation,
        isolationEnveloppe: d.isolationEnveloppe, isolationMurs: d.isolationMurs,
        isolationPlancherBas: d.isolationPlancherBas, isolationMenuiseries: d.isolationMenuiseries,
        dateEtablissement: d.dateEtablissement, dateFinValidite: d.dateFinValidite,
        adresse: d.adresse, codePostal: d.codePostal, commune: d.commune,
        observatoireUrl: d.observatoireUrl,
      },
      audits: auditResults || [],
    })
    setShowDpe(false)
  ...
  function detachDpe() {
    updateProject(id, { dpe: null, audits: null })
  }
              ...
              <div>
                <div className="flex items-center gap-2.5">
                  ...
                  {project.typeTravaux && (() => {
                    ...
                    return tt ? (
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${tt.color}`}>
                        {tt.label}
                      </span>
                    ) : null
                  })()}
              ...
              <div className="bg-slate-50 rounded-xl p-4">
                ...
                {dpe && (
                  <div className="mt-3 pt-3 border-t border-slate-200">
                    <DpeDetailCard dpe={dpe} compact onDetach={() => detachDpe()} />
                  </div>
                )}

                {/* Resultats recherche DPE */}
                {showDpe && dpeResults?.length > 0 && (
                  <div className="mt-2 pt-2 border-t border-slate-200 space-y-1.5 max-h-48 overflow-y-auto">
                    <p className="text-[10px] text-slate-400 font-semibold mb-1">Resultats DPE trouves :</p>
                    {dpeResults.map((d) => (
                      <div key={d.numeroDpe} onClick={(e) => e.stopPropagation()}>
                        <DpeDetailCard dpe={d} compact onAttach={() => selectDpe(d)} />
                      </div>
                    ))}
                  </div>
                )}
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          ...
          <div className="xl:col-span-2 space-y-5">
            ...
            <div className="flex items-center gap-1 bg-white rounded-xl border border-slate-200 p-1">
              {[
                { id: 'scenarios', label: 'Scenarios', icon: Layers, count: (project.scenarios || []).length },
                { id: 'dpe', label: 'DPE & Diagnostic', icon: Thermometer, badge: dpe?.etiquetteDpe },
                { id: 'documents', label: 'Documents', icon: FileText },
              ].map((tab) => (
                <button
                  key={tab.id}
                  onClick={() => setMainTab(tab.id)}
                  className={`flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg text-xs font-semibold transition ${
                    mainTab === tab.id
                ...
                >
                  ...
                  {tab.count != null && (
                    <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${
                      mainTab === tab.id ? 'bg-white/20' : 'bg-slate-100'
                    }`}>{tab.count}</span>
                  )}
                  {tab.badge && (
                    <span
                      className="w-5 h-5 rounded text-[10px] font-black flex items-center justify-center"
                      style={{ backgroundColor: getDpeColor(tab.badge)?.bg, color: getDpeColor(tab.badge)?.text }}
                    >{tab.badge}</span>
                  )}
                </button>
              ))}
            ...
            {mainTab === 'dpe' && (
              <div className="space-y-5">
                {dpe ? (
                  <DpeDetailCard dpe={dpe} audits={project.audits || auditResults} onDetach={detachDpe} />
                ) : (
                  <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-10 text-center">
                    <Thermometer className="w-10 h-10 text-slate-300 mx-auto mb-3" />
                    <p className="text-sm font-medium text-slate-500">Aucun DPE rattache</p>
                    <p className="text-xs text-slate-400 mt-1 mb-4">Recherchez le diagnostic energetique pour ce logement</p>
                    <button
                      onClick={(e) => { e.stopPropagation(); handleSearchDpe() }}
                      disabled={!project.postalCode || dpeLoading}
                      className="inline-flex items-center gap-1.5 px-4 py-2.5 bg-indigo-600 text-white text-xs font-semibold rounded-lg hover:bg-indigo-700 transition disabled:opacity-40"
                    >
                      {dpeLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Search className="w-4 h-4" />}
                      Rechercher le DPE
                    </button>
                  </div>
                )}
                ...
                {showDpe && dpeResults?.length > 0 && (
                  <div className="space-y-2">
                    <p className="text-xs font-semibold text-slate-500">{dpeResults.length} DPE trouve{dpeResults.length > 1 ? 's' : ''} :</p>
                    {dpeResults.map((d) => (
                      <DpeDetailCard key={d.numeroDpe} dpe={d} compact onAttach={() => selectDpe(d)} />
                    ))}
                  </div>
                )}
                {showDpe && dpeResults?.length === 0 && (
                  <p className="text-xs text-slate-400 text-center py-4">Aucun DPE trouve pour cette adresse.</p>
                )}

                {/* Bouton recherche si DPE existe deja */}
                {dpe && (
                  <div className="flex justify-center">
                    <button
                      onClick={(e) => { e.stopPropagation(); handleSearchDpe() }}
                      disabled={!project.postalCode || dpeLoading}
                      className="flex items-center gap-1.5 px-4 py-2 text-xs font-medium text-indigo-600 bg-indigo-50 rounded-lg hover:bg-indigo-100 transition disabled:opacity-40"
                    >
                      {dpeLoading ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Search className="w-3.5 h-3.5" />}
                      Rechercher un autre DPE
                    </button>
                  </div>
                )}
              </div>
          ...
          <div className="space-y-5">
            ...
            <section className="bg-white rounded-xl border border-slate-200 shadow-sm">
              <div className="px-5 py-3.5 border-b border-slate-100">
                <h2 className="text-xs font-semibold text-slate-500 uppercase tracking-wider flex items-center gap-1.5">
                  <Layers className="w-3.5 h-3.5 text-indigo-500" />Type de travaux
                </h2>
              </div>
              <div className="p-4 flex flex-wrap gap-1.5">
                {TYPES_TRAVAUX.map((t) => (
                  <button
                    key={t.value}
                    onClick={() => updateProject(id, { typeTravaux: t.value })}
                    className={`px-3 py-1.5 rounded-full text-xs font-medium transition ${
                      project.typeTravaux === t.value
            ...
            <section className="bg-white rounded-xl border border-slate-200 shadow-sm">
              <div className="px-5 py-3.5 border-b border-slate-100">
                <h2 className="text-xs font-semibold text-slate-500 uppercase tracking-wider flex items-center gap-1.5">
                  <Zap className="w-3.5 h-3.5 text-amber-500" />Suivi du dossier
                </h2>
              </div>
              <div className="p-4">
                {/* Beneficiaire */}
