  ...
  return (
    ...
    >
      ...
      <div className={`p-4 rounded-xl border-2 text-center font-bold text-lg ${
        ...
        {isAnah
          ? `Mode MaPrimeRénov' Parcours Accompagné (DPE ${classInitiale})`
          : `Mode 100% CEE (DPE ${classInitiale})`
        }
      </div>

      <AlertBox type={isAnah ? 'info' : 'warning'} show={true}>
        {isAnah ? (
          <span><strong>DPE {classInitiale}</strong> : financement uniquement via MaPrimeRénov' Parcours Accompagné. Pas de prime CEE sur ce mode.</span>
        ) : (
          <span><strong>DPE {classInitiale}</strong> : financement uniquement via les CEE BAR-TH-174. Pas d'aide ANAH sur ce mode.</span>
        )}
      </AlertBox>
      ...
      <section>
        ...
        <div className="bg-gray-50 p-4 rounded-xl border border-gray-200 space-y-4">
          ...
          {/* Travaux */}
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-2">Catégories de travaux (minimum 2 requises)</label>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {WORK_CATEGORIES_MAISON.map(cat => (
                <label key={cat.value} className={`flex items-center gap-2 p-3 rounded-lg border cursor-pointer transition ${
                  selectedWorks.includes(cat.value) ? 'bg-green-50 border-green-400 text-green-800' : 'bg-white border-gray-200 text-gray-600 hover:border-gray-300'
                }`}>
                  <input type="checkbox" checked={selectedWorks.includes(cat.value)} onChange={() => toggleWork(cat.value)} className="rounded text-green-600" />
          ...
          <AlertBox type="warning" show={isValidJump && !isValidWorks} title="Travaux insuffisants">
            Minimum 2 catégories de travaux doivent être sélectionnées ({selectedWorks.length}/2).
          </AlertBox>
        </div>
      </section>
