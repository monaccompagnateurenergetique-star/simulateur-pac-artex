...
import { Building, Calculator, Euro, TrendingUp, Award, Users, ArrowRight } from 'lucide-react'
...
import {
  ENERGY_CLASSES, TARGET_CLASSES, WORK_GROUPS_APPART, MIN_CLASS_JUMP, CLASS_ORDER,
  CLASSES_ANAH, MPR_INCOME_OPTIONS, MPR_PARCOURS_TAUX, MPR_PARCOURS_PLAFOND, PRIX_CEE_DEFAUT,
  BENEFICIARY_TYPES, OCCUPATION_TYPES, TVA_RATES, getEligibility,
...
export default function BarTh175Page() {
  ...
  const [projectCostHT, setProjectCostHT] = useState(() => getDefault('projectCostHT', 25000))
  const [projectCostTTC, setProjectCostTTC] = useState(() => getDefault('projectCostTTC', 27500))
  ...
  const isolationGroup = WORK_GROUPS_APPART.find(g => g.group === 'isolation')
  ...
  const chauffageGroup = WORK_GROUPS_APPART.find(g => g.group === 'chauffage')
  ...
  const result = useMemo(() => {
    ...
    return calculateRenovationGlobale({
      ...
      projectCostHT: effectiveCostHT, projectCostTTC: effectiveCostTTC,
        ...
        <>
          ...
          <SimulationSaveBar type="BAR-TH-175"
            ...
            pdfData={{
              ...
              ficheTitle: 'Renovation globale — Appartement',
              classInitiale,
              classCible,
              jumps,
              ...
              summary: finalResult.mode === 'anah' ? {
                projectCost: effectiveCostTTC, ceeCommerciale: 0, mprFinal: finalResult.mprAmount,
                totalAid: finalResult.totalAides, resteACharge: finalResult.resteACharge, showMpr: true,
              } : {
                projectCost: effectiveCostTTC, ceeCommerciale: finalResult.ceeFinal, mprFinal: 0,
                totalAid: finalResult.totalAides, resteACharge: finalResult.resteACharge, showMpr: false,
              },
              margin: finalResult.mode === 'cee' ? {
                ceeBase: finalResult.ceeEuros, ceeApplied: finalResult.ceeFinal,
                margin: finalResult.ceeMargin, marginPercent: finalResult.ceeMarginPercent, showOnPdf: false,
